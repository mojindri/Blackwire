...
| Freedom / direct                      | No      | Yes      | **Supported**    | `freedom.rs` — default direct outbound with loopback deny, IP strategy (`Auto`, `UseIP`, prefer/strict IPv4/IPv6), and literal IPv6 rejection controls |
...
| REALITY                                | **Supported**    | `transport/reality/`, `blackwire-core/reality.rs`; e2e `e2e_reality_vless.rs` + transport-only `e2e_reality.rs`; matrix `vless-reality` Xray+sing-box **PASS**; fail-fast client/server handshake timeouts and server-side SNI allow-list checks wired in transport path |
