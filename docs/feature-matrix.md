...
| TUIC v5                               | Yes     | Yes      | **Experimental** | `blackwire-transport/src/tuic.rs` + `blackwire-core/src/tuic.rs`; TCP and native UDP e2e coverage in `e2e_tuic.rs`; Black UI supports `protocol: tuic`. External-client lab proof remains pending before promotion to Supported. |
...
| QUIC (`network: quic` for VLESS/VMess) | **Supported**    | Server: `v2rayquic.rs` + e2e. Matrix: **sing-box PASS**, Xray **SKIP** (Xray 26+ removed legacy QUIC client transport) |
