...
| REALITY                                | **Supported**    | `transport/reality/`, `blackwire-core/reality.rs`; e2e `e2e_reality_vless.rs` + transport-only `e2e_reality.rs`; matrix `vless-reality` Xray+sing-box **PASS**; fail-fast client/server handshake timeouts wired in transport path |
...
| mKCP                                   | **Supported**    | Server: `transport/mkcp/` + e2e. Matrix: both clients **SKIP** (sing-box no mKCP; Xray 26 finalmask) — server support is proven in blackwire e2e and SKIP is an upstream client-model limit |
| QUIC (`network: quic` for VLESS/VMess/Trojan) | **Supported**    | Server: `v2rayquic.rs` + e2e. Matrix: **sing-box PASS**, Xray **SKIP** (Xray 26+ removed legacy QUIC client transport). QUIC TLS advertises `h3`; generated links default QUIC ALPN to `h3`, and VMess QUIC links prefer `aes-128-gcm` body security for client interop. |
| Hysteria2 (QUIC + HTTP/3 auth)         | **Supported**    | `hysteria2/` — TCP stream proxy + UDP datagram path; matrix `hysteria2` Xray+sing-box **PASS**; fail-fast auth/stream timeouts and per-connection UDP worker caps wired in transport path |
...
| `vless-mkcp`                | Yes                 | client rows intentionally SKIP; server support proven by e2e |
