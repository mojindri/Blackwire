...
| Area                                            | Status           | Notes                                                                                                                                                                                                                                                         |
| ----------------------------------------------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Xray / sing-box **wire interop** (as server)    | **Supported**    | Realistic Docker external-client matrix is green with PASS/SKIP-only outcomes (zero FAIL), including `vless-reality`, `hysteria2`, `vless-quic` (sing-box PASS, Xray SKIP), and documented ShadowTLS SKIP rows — see [parity-status.md](parity-status.md) |
...
| Freedom / direct                      | No      | Yes      | **Supported**    | `freedom.rs` — bounded four-attempt Happy Eyeballs with IPv4/IPv6 interleaving, loopback deny, IP strategy (`Auto`, `UseIP`, prefer/strict IPv4/IPv6), and literal IPv6 rejection controls |
...
| VLESS flow `xtls-rprx-vision`         | Yes     | Yes      | **Supported**    | `vless/vision.rs` via common `VisionStream`; authenticated direct handoff removes outer REALITY TLS before raw destination TLS records; portable userspace copy on every platform and optional Linux splice/adaptive splice |
| VMess AEAD                            | Yes     | Yes      | **Supported**    | `vmess/`; legacy **alterId unsupported**; authenticated body lengths match Xray/sing-box semantics and are covered by `vmess::stream::tests` |
...
| Hysteria2                             | Yes     | Yes      | **Supported**    | `blackwire-transport/hysteria2/`, `blackwire-core/hysteria2.rs`; TCP stream relay is supported; UDP datagram relay is implemented but opt-in pending stateful association soak evidence |
| TUIC v5                               | Yes     | Yes      | **Supported**    | `blackwire-transport/src/tuic.rs` + `blackwire-core/src/tuic.rs`; TCP and native UDP e2e coverage in `e2e_tuic.rs`; Black UI supports `protocol: tuic`; external-client coverage is sing-box-oriented because Xray has no TUIC client row in this lab. QUIC/UDP throughput still depends on client, NAT/firewall, MTU, and network path. |
...
| Windows                       | **Supported**   | CI runs Windows workspace tests; core proxy runtime is intended to be portable; Wintun device creation, explicit `wintun.dll` path, split-route setup, packet-level TCP bridging to the local SOCKS listener, and protected outbound interface binding are wired (full-device TUN runtime requires `tun.outboundInterface`/`tun.outbound_interface`) |
...
| Advanced smoke      | `make -C labs/realistic advanced-features-smoke` | ShadowTLS, QUIC/SplitHTTP e2e, health guards + failover runtime |
