...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.41/scripts/install.sh \
  | VERSION=v0.1.41 bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.41/scripts/install.sh \
  | VERSION=v0.1.41 CONFIG_PATH=/path/to/config.json START_SERVICE=1 bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.41/scripts/install.sh \
  | VERSION=v0.1.41 SETUP=reality PUBLIC_HOST=example.com START_SERVICE=1 bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.41/scripts/install.sh \
  | VERSION=v0.1.41 SETUP=domain DOMAIN=proxy.example.com PROXY_PATH=/secret-path INSTALL_NGINX=1 INSTALL_CERTBOT=1 START_SERVICE=1 bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.41/scripts/install.sh \
  | VERSION=v0.1.41 INSTALL_BLACK_UI=1 bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.41/scripts/install.sh \
  | VERSION=v0.1.41 SETUP=domain DOMAIN=proxy.example.com PROXY_PATH=/secret-path INSTALL_NGINX=1 INSTALL_CERTBOT=1 INSTALL_BLACK_UI=1 START_SERVICE=1 bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.41/scripts/install.sh \
  | VERSION=v0.1.41 ACTION=upgrade bash
```
...
If the client log contains `write udp ... no buffer space available`, the local
client UDP socket queue is overloaded. Blackwire's default Hysteria2 mode uses
standard QUIC congestion with bounded flow-control windows and does not
advertise a fixed Hysteria2 auth bandwidth target. Use the Hysteria2
`Throughput` mode or explicit `settings.upMbps` / `settings.downMbps` only when
the client path can absorb larger bursts.
...
```sh
curl -4 -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.41/scripts/install.sh \
  | VERSION=v0.1.41 bash
```
