...
This is a pre-1.0 project. The project as a whole is **not yet production-ready**,
...
- SOCKS5 inbound (TCP CONNECT + UDP ASSOCIATE), HTTP CONNECT inbound, Freedom outbound with IP strategy and literal IPv6 guard controls
...
prereleases; final tags such as `v0.1.36` are published as stable GitHub releases.
...
```sh
gh workflow run release-assets.yml -f tag=v0.1.36
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.36/scripts/install.sh \
  | VERSION=v0.1.36 bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.36/scripts/install.sh \
  | VERSION=v0.1.36 CONFIG_PATH=/path/to/config.json bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.36/scripts/install.sh \
  | VERSION=v0.1.36 SETUP=reality PUBLIC_HOST=example.com bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.36/scripts/install.sh \
  | VERSION=v0.1.36 SETUP=domain DOMAIN=proxy.example.com PROXY_PATH=/secret-path INSTALL_NGINX=1 INSTALL_CERTBOT=1 START_SERVICE=1 bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.36/scripts/install.sh \
  | VERSION=v0.1.36 INSTALL_BLACK_UI=1 bash
```
