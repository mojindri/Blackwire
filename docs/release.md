...
prereleases; final tags such as `v0.1.29` are published as stable GitHub releases.
...
```sh
gh workflow run release-assets.yml -f tag=v0.1.29
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.29/scripts/install.sh \
  | VERSION=v0.1.29 CONFIG_PATH=/path/to/config.json bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.29/scripts/install.sh \
  | VERSION=v0.1.29 SETUP=reality PUBLIC_HOST=example.com bash
```
