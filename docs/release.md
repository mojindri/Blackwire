...
prereleases; final tags such as `v0.1.28` are published as stable GitHub releases.
...
```sh
gh workflow run release-assets.yml -f tag=v0.1.28
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.28/scripts/install.sh \
  | VERSION=v0.1.28 CONFIG_PATH=/path/to/config.json bash
```
...
```sh
curl -fsSL https://raw.githubusercontent.com/mojindri/Blackwire/v0.1.28/scripts/install.sh \
  | VERSION=v0.1.28 SETUP=reality PUBLIC_HOST=example.com bash
```
