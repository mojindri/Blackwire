...
| VMess/xHTTP connects, logs decoded VMess headers, then client DNS fails with EOF and server relays `uplink_bytes=0 downlink_bytes=0` | Body framing mismatch after the header. For clients that set `authenticated_length: true`, the authenticated length plaintext is payload plus padding length; the AEAD tag length is added by the parser/writer, not included in the encrypted length value. | `vmess/stream.rs`; regression `authenticated_length_encodes_plain_payload_plus_padding_size`; rerun `cargo test -p blackwire-protocol vmess::stream::tests -- --nocapture` and `cargo test -p integration-tests --test e2e_vmess -- --nocapture` |
