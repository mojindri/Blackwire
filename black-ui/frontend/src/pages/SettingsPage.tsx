...
export function SettingsPage({ settings, coreSettings, busy, onSave, onSaveCore }: { settings: Settings | null; coreSettings: CoreSettings | null; busy: boolean; onSave: (settings: Settings) => void; onSaveCore: (settings: CoreSettings) => void }) {
  ...
  return <div className="page settings-page">
    ...
    <SettingsSection icon={<Gauge size={19} />} title="QUIC transport" copy="Optional host-specific QUIC socket tuning." recommendation="Leave overrides off so Blackwire can size buffers and endpoint shards from available CPU and memory." impact="Resource-aware · automatic" details="Native Hysteria2 datagrams are managed automatically. FEC stays off because arbitrary peers do not negotiate Blackwire-specific recovery framing. Automatic QUIC sizing is capped conservatively; explicit socket values always win.">
      <div className="settings-toggle-strip"><Switch checked={core.quic !== null} onChange={(enabled) => setCore({ ...core, quic: enabled ? defaultQuic : null })} label="QUIC socket overrides" /></div>
