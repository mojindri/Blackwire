...
import { ChevronDown, Gauge, Network, Save, Settings2, Shield, Terminal } from "lucide-react";
...
export function SettingsPage({ settings, coreSettings, busy, onSave, onSaveCore }: { settings: Settings | null; coreSettings: CoreSettings | null; busy: boolean; onSave: (settings: Settings) => void; onSaveCore: (settings: CoreSettings) => void }) {
  ...
  return <div className="page settings-page">
    ...
    <SettingsSection icon={<Network size={19} />} title="TUN interception" copy="Interface, batching, and session controls." recommendation="Leave TUN disabled unless Blackwire must capture host or device traffic. Confirm routes, bypass marks and the outbound interface before saving." impact="Expert · automatic" details="TUN creates a virtual network interface and redirects traffic into Blackwire. Changes use a prepared in-process handover and keep the last working configuration if preparation fails.">
...
function TunEditor({ value, onChange }: { value: NonNullable<CoreSettings["tun"]>; onChange: (value: NonNullable<CoreSettings["tun"]>) => void }) {
  const batch = value.batch, sessions = value.sessions;
