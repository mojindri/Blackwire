import { CirclePlay, RotateCcw, Square, Wrench } from "lucide-react";
import type { RevisionSummary, ServiceStatus, Status } from "../lib/types";
import { Badge } from "../components/atoms/Badge";
import { Button } from "../components/atoms/Button";

export function ServicePage({ status, service, revisions, busy, onStart, onStop, onRestart, onRollback, onActivateMaintenance }: {
  ...
  return <div className="page">
    ...
    <section className="work-panel runtime-service-panel"><div className="panel-toolbar"><div><h2>Blackwire service</h2><p>Host service state and recent runtime output.</p></div><Badge tone={service?.activeState === "active" ? "green" : service?.systemdAvailable ? "amber" : "gray"}>{service?.activeState ?? "unknown"}</Badge></div><div className="runtime-service-actions"><Button variant="secondary" icon={<CirclePlay size={15} />} disabled={busy || !service?.systemdAvailable || service.activeState === "active"} onClick={onStart}>Start</Button><Button variant="secondary" icon={<Square size={14} />} disabled={busy || !service?.systemdAvailable || service.activeState !== "active"} onClick={onStop}>Stop</Button><Button variant="primary" icon={<RotateCcw size={15} />} disabled={busy || !service?.systemdAvailable} onClick={onRestart}>Restart</Button><span>{service?.subState ?? "Service manager unavailable"}</span></div><pre className="runtime-log-view">{service?.logs.length ? service.logs.join("\n") : "No journal entries available on this host."}</pre></section>
