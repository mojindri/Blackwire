...
fn seed_default_outbound(conn: &Connection) -> Result<()> {
    ...
    conn.execute(
        "INSERT OR IGNORE INTO outbounds (tag, protocol, enabled, settings, stream_settings, created_at, updated_at)
         VALUES ('freedom', 'freedom', 1, '{\"denyLoopback\":true,\"domainStrategy\":\"PreferIPv4\",\"rejectIpv6Literal\":false}', '', ?1, ?1)",
        params![ts],
    )?;
    Ok(())
}
...
mod tests {
    ...
    fn init_migrates_prototype_schema_without_losing_rows() {
        ...
        assert_eq!(settings["rejectIpv6Literal"], false);
    ...
    fn public_link_env_migration_replaces_local_defaults_only() {
        ...
        migrate_local_public_settings(&conn, "http://169.40.15.126:18080", "169.40.15.126")
            .unwrap();
        let migrated = load_settings(&conn).unwrap();
        assert_eq!(migrated.public_base_url, "http://169.40.15.126:18080");
        assert_eq!(migrated.subscription_host, "169.40.15.126");
