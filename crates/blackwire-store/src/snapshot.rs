...
async fn load_inbounds(pool: &MySqlPool, revision: i64) -> StoreResult<Vec<InboundConfig>> {
    ...
    for row in rows {
        ...
        let settings = match protocol {
            Protocol::Tuic => json!({ "users": clients }),
            Protocol::Hysteria2 => {
                let auth = clients.first().and_then(|v| v.get("auth")).cloned();
                let mut value = json!({ "clients": clients });
                if let Some(auth) = auth { value["auth"] = auth; }
                value
            }
            Protocol::Vless | Protocol::Vmess | Protocol::Trojan | Protocol::Shadowsocks => json!({ "clients": clients }),
            _ => json!({}),
        };
        result.push(InboundConfig {
            tag: row.try_get("tag")?,
            protocol,
            listen: row.try_get::<String, _>("listen_address")?.parse::<IpAddr>().map_err(decode_error)?,
...
async fn load_outbounds(pool: &MySqlPool, revision: i64) -> StoreResult<Vec<OutboundConfig>> {
    ...
    for row in rows {
        ...
        insert_optional(&mut settings, "address", row.try_get::<Option<String>, _>("server_address")?);
        insert_optional(&mut settings, "port", row.try_get::<Option<u32>, _>("server_port")?);
        insert_optional(&mut settings, "domainStrategy", row.try_get::<Option<String>, _>("domain_strategy")?);
        insert_optional(&mut settings, "denyLoopback", row.try_get::<Option<bool>, _>("deny_loopback")?);
        insert_optional(&mut settings, "rejectIpv6Literal", row.try_get::<Option<bool>, _>("reject_ipv6_literal")?);
...
async fn load_clients(pool: &MySqlPool, revision: i64, inbound_id: i64, protocol: &Protocol) -> StoreResult<Vec<Value>> {
    ...
    rows.into_iter().map(|row| {
        ...
        Ok(match protocol {
            Protocol::Vless | Protocol::Vmess => json!({"id": uuid.unwrap_or_default(), "email": email, "flow": row.try_get::<String, _>("flow")?}),
            Protocol::Trojan => json!({"password": password.unwrap_or_default(), "email": email}),
            Protocol::Shadowsocks => json!({"password": password.unwrap_or_default(), "email": email, "method": row.try_get::<Option<String>, _>("method")?}),
            Protocol::Hysteria2 => json!({"auth": auth.or(password).unwrap_or_default(), "email": email}),
            Protocol::Tuic => json!({"uuid": uuid.unwrap_or_default(), "password": password.unwrap_or_default(), "email": email}),
            _ => json!({"email": email}),
        })
    }).collect()
}

async fn load_stream(pool: &MySqlPool, revision: i64, kind: &str, id: i64) -> StoreResult<Option<StreamSettingsConfig>> {
    ...
    let mut stream = StreamSettingsConfig { network: parse_network(&row.try_get::<String, _>("network")?)?, security: parse_security(&row.try_get::<String, _>("security")?)?, ..Default::default() };
...
async fn load_dns(pool: &MySqlPool, revision: i64) -> StoreResult<Option<DnsConfig>> {
    let Some(row) = sqlx::query("SELECT enabled, fake_ip_enabled, fake_ip_pool FROM dns_config WHERE revision_id = ?")
        .bind(revision).fetch_optional(pool).await? else { return Ok(None); };
    if !row.try_get::<bool, _>("enabled")? { return Ok(None); }
    let servers = sqlx::query_scalar("SELECT address FROM dns_servers WHERE revision_id = ? ORDER BY position").bind(revision).fetch_all(pool).await?;
    let fake_ip = row.try_get::<bool, _>("fake_ip_enabled")?.then(|| FakeIpConfig { enabled: true, pool: row.try_get::<Option<String>, _>("fake_ip_pool").ok().flatten().unwrap_or_else(|| "198.18.0.0/15".into()) });
...
async fn load_routing(pool: &MySqlPool, revision: i64) -> StoreResult<Option<RoutingConfig>> {
    ...
    if !config.try_get::<bool, _>("enabled")? { return Ok(None); }
    ...
    for row in rows {
        ...
        let mut rule = RoutingRule { rule_type: row.try_get("rule_type")?, port: row.try_get("port_expression")?, ..Default::default() };
        for value in values {
            let text: String = value.try_get("value_text")?;
            match value.try_get::<String, _>("value_kind")?.as_str() { "domain" => rule.domain.push(text), "ip" => rule.ip.push(text), "inbound_tag" => rule.inbound_tag.push(text), "protocol" => rule.protocol.push(text), "user" => rule.user.push(text), _ => {} }
        }
        rule.outbound_tag = sqlx::query_scalar("SELECT tag FROM outbounds WHERE revision_id = ? AND outbound_id = ?").bind(revision).bind(row.try_get::<i64, _>("outbound_id")?).fetch_one(pool).await?;
        rules.push(rule);
    }
    Ok(Some(RoutingConfig { domain_strategy: config.try_get("domain_strategy")?, geoip_file: config.try_get("geoip_file")?, geosite_file: config.try_get("geosite_file")?, rules, balancers: Vec::new() }))
}

fn optional_usize(row: &sqlx::mysql::MySqlRow, name: &str) -> StoreResult<Option<usize>> { row.try_get::<Option<u64>, _>(name)?.map(usize::try_from).transpose().map_err(decode_error) }
fn bytes_string(row: &sqlx::mysql::MySqlRow, name: &str) -> StoreResult<Option<String>> { row.try_get::<Option<Vec<u8>>, _>(name)?.map(String::from_utf8).transpose().map_err(decode_error) }
fn insert_optional<T: serde::Serialize>(map: &mut serde_json::Map<String, Value>, key: &str, value: Option<T>) { if let Some(value) = value { map.insert(key.into(), json!(value)); } }
fn decode_error(error: impl std::error::Error + Send + Sync + 'static) -> StoreError { StoreError::Sql(sqlx::Error::Decode(Box::new(error))) }

fn parse_profile(value: &str) -> StoreResult<ProfileMode> { match value { "compat" => Ok(ProfileMode::Compat), "fast" => Ok(ProfileMode::Fast), "latency" => Ok(ProfileMode::Latency), "throughput" => Ok(ProfileMode::Throughput), "badnet" => Ok(ProfileMode::Badnet), "mobile" => Ok(ProfileMode::Mobile), "stealth" => Ok(ProfileMode::Stealth), other => Err(value_error("profile", other)) } }
fn parse_protocol(value: &str) -> StoreResult<Protocol> { match value { "vless" => Ok(Protocol::Vless), "vmess" => Ok(Protocol::Vmess), "trojan" => Ok(Protocol::Trojan), "shadowsocks" => Ok(Protocol::Shadowsocks), "hysteria2" => Ok(Protocol::Hysteria2), "tuic" => Ok(Protocol::Tuic), "socks" => Ok(Protocol::Socks), "http" => Ok(Protocol::Http), "freedom" => Ok(Protocol::Freedom), other => Err(value_error("protocol", other)) } }
fn parse_network(value: &str) -> StoreResult<NetworkType> { match value { "tcp" => Ok(NetworkType::Tcp), "ws" => Ok(NetworkType::Ws), "httpupgrade" => Ok(NetworkType::HttpUpgrade), "grpc" => Ok(NetworkType::Grpc), "quic" => Ok(NetworkType::Quic), "kcp" => Ok(NetworkType::Kcp), "splithttp" | "xhttp" => Ok(NetworkType::SplitHttp), other => Err(value_error("network", other)) } }
fn parse_security(value: &str) -> StoreResult<SecurityType> { match value { "none" => Ok(SecurityType::None), "tls" => Ok(SecurityType::Tls), "reality" => Ok(SecurityType::Reality), "shadowtls" => Ok(SecurityType::ShadowTls), other => Err(value_error("security", other)) } }
fn value_error(kind: &str, value: &str) -> StoreError { decode_error(ValueError(format!("invalid {kind} '{value}'"))) }

#[derive(Debug)] struct ValueError(String);
impl std::fmt::Display for ValueError { fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { f.write_str(&self.0) } }
