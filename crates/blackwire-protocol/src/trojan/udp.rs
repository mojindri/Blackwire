...
///
/// Exchange is sequential (read datagram → forward to UDP → await reply → write reply),
/// so a separate write task adds no real parallelism. The reply is written inline using
/// a reusable encode buffer, eliminating one heap allocation per reply.
pub async fn relay_trojan_udp(
    ...
    // Pre-allocated buffers — reused across every datagram exchange.
