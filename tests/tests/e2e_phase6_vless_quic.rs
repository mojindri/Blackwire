...
fn write_temp_tls_files() -> (PathBuf, PathBuf) {
    ...
    let cert_path = std::env::temp_dir().join(format!("blackwire-quic-{suffix}.crt"));
    let key_path = std::env::temp_dir().join(format!("blackwire-quic-{suffix}.key"));
    ...
    parse_config(format!(
        r#"{{
            "inbounds": [{{
                ...
                "port": {vless_port},
                "settings": {{
                    "clients": [{{ "id": "{TEST_UUID}", "email": "quic@example.test" }}]
                }},
                "streamSettings": {{
                    "network": "quic",
                    "tlsSettings": {{
...
fn client_config(socks_port: u16, vless_port: u16) -> Arc<blackwire_config::schema::Config> {
    parse_config(format!(
        r#"{{
            ...
            "routing": {{ "rules": [{{ "outboundTag": "vless-quic-out" }}] }}
        }}"#
    ))
}

#[tokio::test]
async fn phase6_vless_quic_echo() {
    let (cert_path, key_path) = write_temp_tls_files();
