//! VLESS with XTLS Vision flow over plain TCP (blackwire ↔ blackwire).
    ...
    parse_config(format!(
        r#"{{
            ...
            "inbounds": [{{
                ...
                "port": {vless_port},
                "settings": {{
                    "clients": [{{ "id": "{TEST_UUID}", "email": "vmess-grpc@example.test", "flow": "{VISION_FLOW}" }}]
                }}
            }}],
        ...
        r#"{{
            ...
            "outbounds": [{{
                ...
                "settings": {{
                    "address": "127.0.0.1",
                    "port": {vless_port},
                    "users": [{{ "id": "{TEST_UUID}", "flow": "{VISION_FLOW}" }}]
                }}
            }}],
            "routing": {{ "rules": [{{ "outboundTag": "vless-vision-out" }}] }}
        }}"#
    ))
}

#[tokio::test]
async fn vless_vision_plain_tcp_echo() {
