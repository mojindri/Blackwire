...
target="http://${SERVER_HOST}:${COMPETITIVE_REMOTE_UPSTREAM_PORT}/${TUN_TCP_PAYLOAD}"
if curl_raw="$(curl -fsS -o /dev/null -w 'speed_download=%{speed_download}\ntime_total=%{time_total}\nsize_download=%{size_download}\n' "$target" 2>&1)"; then
...
RAW_TEXT="$curl_raw" python3 - "$VARIANT" "$SCENARIO" "$TUN_TCP_PAYLOAD" "$COMPETITIVE_CONCURRENCY" "$COMPETITIVE_DURATION" "$TS" "$LOSS_PERCENT" "$RTT_MS" "$JITTER_MS" "${VARIANT}-cpu.log" <<'PY'
import json, os, re, sys
variant, scenario, payload, conc, duration, ts, loss, rtt, jitter, cpu_file = sys.argv[1:]
...
row = {
    ...
    "reason": "" if speed_bps > 0 else raw[:200],
}
