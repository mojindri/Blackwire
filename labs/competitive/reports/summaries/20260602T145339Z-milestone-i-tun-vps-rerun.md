...
After manual reboot, client VPS `<client-host>` was reachable again and its route table was clean.
...
Additional checks showed that client VPS `<client-host>` refuses outbound TCP to several tested targets even with no TUN active:
...
The server VPS `<server-host>` has normal outbound TCP. This means the planned client-side TCP benchmark cannot satisfy the acceptance gate until the VPS/network/firewall policy for outbound TCP on `<client-host>` is corrected, or the benchmark roles are moved to a host with normal TCP egress and a reachable upstream target.
