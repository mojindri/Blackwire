# Hysteria2 Client/Server Example

This example shows the Hysteria2 QUIC data path:
...
```sh
cargo test -p integration-tests hysteria2_to_freedom_transfers_data
```
...
```sh
cargo run -p blackwire -- test -c examples/hysteria2-client-server/server.json
cargo run -p blackwire -- test -c examples/hysteria2-client-server/client.json
```
